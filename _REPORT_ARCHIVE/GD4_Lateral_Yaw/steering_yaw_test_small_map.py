...
def safety_check(vehicle, spawn_transform):
    ...
    if (
        ...
        raise RuntimeError(
            "Vehicle bị rơi khỏi map: z={:.3f}.".format(
                float(location.z)
            )
        )
...
def main():
    ...
    try:
        ...
        print("=" * 96)
        print(
            "GĐ4 - TEST 4 STEERING / YAW - SMALL MAP"
        )
        print("=" * 96)
        ...
        print(
            "STEER MAP : ±12 deg -> normalized ±{:.6f}".format(
                abs(steer_deg_to_normalized(12.0))
            )
        )
        print("=" * 96)
        ...
        print("")
        print(
            "HOÀN TẤT TEST 4 BASELINE. "
            "Gửi toàn bộ terminal output để quyết định có cần tune lateral hay không."
        )
