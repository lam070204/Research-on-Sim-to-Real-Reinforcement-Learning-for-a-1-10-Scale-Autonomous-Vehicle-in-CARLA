...
# ----------------------------------------------------------------
    ...
    def get_next_spawn_transform(self):
        """
        Chọn cố định một spawn point để kiểm tra và thu dataset.

        Đổi SPAWN_NUMBER từ 1 đến tổng số spawn point của map.
        Ví dụ:
            SPAWN_NUMBER = 1  -> self.spawn_points[0]
            SPAWN_NUMBER = 8  -> self.spawn_points[7]
        """
        SPAWN_NUMBER = 1

        index = SPAWN_NUMBER - 1

        if index < 0 or index >= len(self.spawn_points):
            raise RuntimeError(
                "SPAWN_NUMBER={} không hợp lệ. Map chỉ có {} spawn point.".format(
                    SPAWN_NUMBER,
                    len(self.spawn_points),
                )
            )
        ...
        print(
            "\nĐang sử dụng spawn point {}/{}".format(
                SPAWN_NUMBER,
                len(self.spawn_points),
            )
        )

        # Tạo bản sao transform để không sửa trực tiếp spawn point của map.
        # Nâng xe lên 0.20 m để hạn chế bánh xe xuyên mặt đường khi spawn.
