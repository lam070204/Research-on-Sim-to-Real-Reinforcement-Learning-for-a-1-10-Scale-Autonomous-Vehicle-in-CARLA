...
class CarlaEnvironmentRGBVisionDRV4(CarlaEnvironmentRGBV3Base):
    def __init__(self, *args, **kwargs):
        # SIM-TO-REAL SAFE switches.
        # Dynamics DR remains controlled by the V3 kwarg:
        #     domain_randomization_enabled
        # Vision groups are intentionally independent so ablations are possible.
        self.camera_pose_dr_enabled = bool(
            kwargs.pop("camera_pose_dr_enabled", False)
        )
        self.weather_dr_enabled = bool(
            kwargs.pop("weather_dr_enabled", False)
        )
        self.image_dr_enabled = bool(
            kwargs.pop("image_dr_enabled", False)
        )
        self.sensor_noise_dr_enabled = bool(
            kwargs.pop("sensor_noise_dr_enabled", False)
        )
        self.vision_dr_enabled = bool(
            self.camera_pose_dr_enabled
            or self.weather_dr_enabled
            or self.image_dr_enabled
            or self.sensor_noise_dr_enabled
        )

        speed_cap = kwargs.pop("policy_speed_cap_mps", 0.35)
        self.policy_speed_cap_mps = (
            None if speed_cap is None else float(speed_cap)
        )
        if (
            self.policy_speed_cap_mps is not None
            and self.policy_speed_cap_mps <= 0.0
        ):
            raise ValueError("policy_speed_cap_mps must be > 0 or None.")
        ...
        print(
            "ENV V4 SAFE | vision={} | camera_pose={} | weather={} | "
            "image={} | sensor_noise={} | speed_cap={}".format(
                self.vision_dr_enabled,
                self.camera_pose_dr_enabled,
                self.weather_dr_enabled,
                self.image_dr_enabled,
                self.sensor_noise_dr_enabled,
                self.policy_speed_cap_mps,
            )
        )
    ...
    def _sample_v4_episode(self):
        ...
        if not self.camera_pose_dr_enabled:
            for key in (
                "camera_dx_real_m",
                "camera_dy_real_m",
                "camera_dz_real_m",
        ...
        if not self.image_dr_enabled:
            for key in (
                ...
                p[key] = 1.0
            p["gaussian_noise_sigma"] = 0.0

        if not self.sensor_noise_dr_enabled:
            for key in (
                "speed_bias_mps",
                "speed_noise_std_mps",
                "yaw_bias_rad_s",
    ...
    def _apply_v4_weather(self):
        if not self.weather_dr_enabled:
            return
    ...
    def _build_observation(self):
        ...
        if (
            ...
            if self.image_dr_enabled:
                ...
                augmented_rgb = np.ascontiguousarray(raw_rgb)
        ...
        if self.sensor_noise_dr_enabled:
            ...
            policy_state = {
                "speed_mps": float(clean_imu_state["speed_mps"]),
                "yaw_rate_rad_s": float(clean_imu_state["yaw_rate_rad_s"]),
                "longitudinal_accel_mps2": float(
                    clean_imu_state["longitudinal_accel_mps2"]
                ),
            }
    ...
    def step(self, action):
        ...
        if raw_action.size != 2:
            raise ValueError(
                "Expected action [steer_cmd, speed_cmd_mps], got shape {}.".format(
                    tuple(raw_action.shape)
                )
            )

        applied_action = raw_action.copy()
        applied_action[0] = np.clip(applied_action[0], -1.0, 1.0)

        if self.policy_speed_cap_mps is not None:
            applied_action[1] = np.clip(
                applied_action[1],
                0.0,
                self.policy_speed_cap_mps,
            )
        ...
        info["policy_action_raw_safe"] = (
            float(raw_action[0]),
            float(raw_action[1]),
        )
        info["policy_action_applied_safe"] = (
            float(applied_action[0]),
            float(applied_action[1]),
        )
        info["policy_speed_cap_mps"] = self.policy_speed_cap_mps
