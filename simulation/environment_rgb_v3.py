...
from simulation.sensors_v3 import (
    CameraSensor,
    CameraSensorEnv,
    CollisionSensor,
    RGBDatasetCamera,
...
class CarlaEnvironmentRGB():
    ...
    def __init__(
        ...
        self.rgb_dataset_save_every = max(1, int(rgb_dataset_save_every))
        self.rgb_dataset_max_images = max(1, int(rgb_dataset_max_images))
        self.rgb_dataset_test_interval = max(
            2, int(rgb_dataset_test_interval)
        )
        ...
        print(
            "ENVIRONMENT: RGB V3 | "
            "NAV=[applied_throttle, normalized_speed, applied_steer, "
            "signed_lateral, signed_heading]"
        )
    ...
    def reset(self):
        ...
        try:
            ...
            # Geometry + image settings lấy duy nhất từ sensors_v3.py.
            self.camera_obj = CameraSensor(self.vehicle)
            self.wait_for_camera_frame(self.camera_obj)
            self.image_obs = self.camera_obj.front_camera.pop(-1)
            self.sensor_list.append(self.camera_obj.sensor)

            # Camera RGB thứ hai chỉ tạo khi bật thu dataset V3.
            # Nó dùng cùng geometry/image settings với camera PPO.
            if self.collect_rgb_dataset:
                self.rgb_dataset_obj = RGBDatasetCamera(
                    self.vehicle,
                    save_every=self.rgb_dataset_save_every,
                    max_images=self.rgb_dataset_max_images,
                    test_interval=self.rgb_dataset_test_interval,
                )
            ...
            self.signed_distance_from_center = float(0.0)
            ...
            self.navigation_obs = self.build_navigation_observation()
            time.sleep(0.5)
    ...
    def step(self, action_idx):
        try:
            ...
            if self.continous_action_space:
                ...
                # Lưu đúng lệnh thực tế đã gửi xuống CARLA.
                self.previous_steer = float(smooth_steer)
                self.throttle = float(smooth_throttle)
            else:
            ...
            route_point_a = self.vector(
                self.current_waypoint.transform.location
            )
            route_point_b = self.vector(
                self.next_waypoint.transform.location
            )
            vehicle_point = self.vector(self.location)

            self.signed_distance_from_center = (
                self.signed_distance_to_line(
                    route_point_a,
                    route_point_b,
                    vehicle_point,
                )
            )
            # Reward và điều kiện OFF ROAD vẫn dùng độ lớn dương.
            self.distance_from_center = abs(
                self.signed_distance_from_center
            )
            self.center_lane_deviation += self.distance_from_center

            # Heading error có dấu.
            # Dùng hướng đầu xe thay cho vector vận tốc để ổn định lúc xe chậm.
            vehicle_fwd = self.vector(
                self.vehicle.get_transform().get_forward_vector()
            )
            waypoint_fwd = self.vector(
            ...
            self.image_obs = self.camera_obj.front_camera.pop(-1)
            self.navigation_obs = (
                self.build_navigation_observation()
            )
...
# ----------------------------------------------------------------
    ...
    def signed_distance_to_line(self, A, B, p):
        """
        Khoảng cách vuông góc có dấu từ xe tới centerline.

        Quy ước theo hướng route A -> B:
        - giá trị dương: xe nằm bên trái route;
        - giá trị âm: xe nằm bên phải route.
        """
        point_a = np.asarray(A, dtype=np.float32)[:2]
        point_b = np.asarray(B, dtype=np.float32)[:2]
        point_p = np.asarray(p, dtype=np.float32)[:2]
        ...
        relative = point_p - point_a
        cross_z = (
            float(segment[0]) * float(relative[1])
            - float(segment[1]) * float(relative[0])
        )

        return cross_z / denominator


    def build_navigation_observation(self):
        """
        Navigation state V3 cố định ở reset() và step():

        0. throttle thực tế đã áp dụng, khoảng 0..1
        1. tốc độ chuẩn hóa theo target_speed
        2. steer thực tế đã áp dụng, khoảng -1..1
        3. sai lệch ngang có dấu, chuẩn hóa về -1..1
        4. sai lệch heading có dấu, chuẩn hóa về -1..1
        """
        target_speed = max(float(self.target_speed), 1e-6)
        lane_limit = max(float(self.max_distance_from_center), 1e-6)
        heading_limit = np.deg2rad(20.0)

        normalized_speed = float(
            np.clip(
                float(self.velocity) / target_speed,
                0.0,
                1.5,
            )
        )
        normalized_lateral = float(
            np.clip(
                float(self.signed_distance_from_center) / lane_limit,
                -1.0,
                1.0,
            )
        )
        normalized_heading = float(
            np.clip(
                float(self.angle) / heading_limit,
                -1.0,
                1.0,
            )
        )
