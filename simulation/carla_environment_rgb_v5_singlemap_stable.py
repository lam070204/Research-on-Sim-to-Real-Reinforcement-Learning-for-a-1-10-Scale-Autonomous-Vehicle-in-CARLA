# -*- coding: utf-8 -*-
"""
CARLA RGB V5 SINGLE-MAP STABLE / CONTROLLED DR
...
Action:
    ...
    steer_cmd       in [-1, +1]
    action[1]       in [0, 1] from PPO; env scales it to [0, policy_speed_max_mps]
...
from stable_action_adapter_v5 import StableActionAdapterV5

from domain_randomization_v5_singlemap_stable import (
...
from reward_manager_v5_singlemap_stable import (
    RewardManagerV5,
    EpisodeConfigV5,
)
...
# SINGLE-MAP STABLE interprets desired_speed_mps as HARD MAX speed.
DEFAULT_DESIRED_SPEED_MPS = 0.35
...
class CarlaEnvironmentRGBV5(object):
    ...
    def __init__(
        ...
        self.policy_speed_max_mps = _clip(
            policy_speed_max_mps,
            0.01,
            float(REAL.max_speed_mps),
        )
        self.steer_deadband = _clip(
            steer_deadband,
            0.0,
            0.25,
        )
        self.dr_family_budget = max(
            1,
            int(dr_family_budget),
        )

        # Logical command smoothing is applied BEFORE dynamics DR so the policy
        # observation sees the same command that would be transmitted on the
        # real vehicle. Do not hide smoothing inside CARLA-only physics.
        self._stable_prev_steer_cmd = 0.0
        self._stable_prev_speed_cmd_mps = 0.0

        self.stable_action_adapter = StableActionAdapterV5(
            speed_max_mps=self.policy_speed_max_mps,
            steer_deadband=self.steer_deadband,
        ...
        self._episode_dynamics_dr_enabled = False
        self._episode_vision_dr_enabled = False
        self._episode_sensor_dr_enabled = False
        self._episode_dr_families = tuple()
        ...
        print(
            "ENV RGB V5 SINGLEMAP LOOKAHEAD | obs={} | action=[steer,speed] | "
            "Hz={:.1f} | maxSpeed={:.3f} m/s | steerRate={} /s | deadband={:.3f} | "
            "dynamicsDR={} visionDR={} sensorDR={} | "
            "DRmix={:.0f}% randomized / {:.0f}% nominal | budget={} family/episode | "
            "recovery={} | spawns={}".format(
                OBSERVATION_DIM_V5,
                self.control_hz,
                self.policy_speed_max_mps,
                self.steer_rate_limit_per_s,
                self.steer_deadband,
    ...
    def set_stable_dr_control(
        ...
        """
        Update the CONTROLLED-DR schedule between PPO rollouts.

        Important:
        - The new setting is sampled on the next reset.
        - At most `family_budget` DR families are active in one randomized episode.
        - Nominal episodes remain exact nominal.
        """
        self.dr_episode_probability = _clip(
            probability,
            0.0,
    ...
    def _prepare_episode_dynamics_dr(self):
        if self._episode_dynamics_dr_enabled:
            params = (
                sample_domain_randomization_v5(
    ...
    def _sample_vision_sensor_episode(self):
        ...
        if (
            ...
            or (
                not self._episode_vision_dr_enabled
                and not self._episode_sensor_dr_enabled
            )
        ):
            self.vision_sensor_params = (
                nominal_vision_domain_v5()
        ...
        if not self._episode_vision_dr_enabled:
            vision_keys = (
                "camera_dx_real_m",
                "camera_dy_real_m",
                "camera_dz_real_m",
        ...
        if not self._episode_sensor_dr_enabled:
            sensor_keys = (
                "speed_bias_mps",
                "speed_noise_std_mps",
                "yaw_bias_rad_s",
    ...
    def _spawn_episode_components(self):
        if self._episode_vision_dr_enabled:
            camera_transform = (
    ...
    def _build_observation(self):
        ...
        if self._episode_sensor_dr_enabled:
            policy_state = (
                noisy_state_for_policy_v5(
                    clean_imu_state=(
                        clean_imu_state
    ...
    def reset(self):
        try:
            ...
            self._stable_prev_steer_cmd = 0.0
            self._stable_prev_speed_cmd_mps = 0.0
            self.stable_action_adapter.reset()

            # Controlled DR:
            # randomized episodes are mixed with exact nominal episodes and,
            # critically, only a small number of DR families may be active in
            # one episode. This prevents the policy from learning to "fight"
            # several unrelated perturbations at the same time.
            available_families = []
            if self.dynamics_dr_enabled:
                available_families.append("dynamics")
            if self.vision_dr_enabled:
                available_families.append("vision")
            if self.sensor_dr_enabled:
                available_families.append("sensor")
            ...
            selected = []
            if self._episode_dr_active:
                budget = min(
                    int(self.dr_family_budget),
                    len(available_families),
                )
                if budget >= len(available_families):
                    selected = list(available_families)
                else:
                    selected = self._domain_rng.sample(
                        available_families,
                        budget,
                    )
            ...
            self._episode_dynamics_dr_enabled = bool(
                "dynamics" in selected
            )
            self._episode_vision_dr_enabled = bool(
                "vision" in selected
            )
            self._episode_sensor_dr_enabled = bool(
                "sensor" in selected
            )
            ...
            print(
                ...
                "obs={} | speed={:.3f}m/s".format(
                    ...
                    (
                        "+".join(self._episode_dr_families).upper()
                        if self._episode_dr_active
                        else "NOMINAL"
                    ),
    ...
    def step(self, action):
        ...
        try:
            ...
            # TRAIN-ONLY privileged target.  Compute it before applying this
            # PPO action so reward compares the action with the oracle action
            # for the state in which the decision was made.
            (
                teacher_steer_target_unit,
                teacher_preview_error_rad,
                teacher_lookahead_distance_m,
            ) = self.reward_manager.oracle_steer_target_unit()

            stable_cmd = self.stable_action_adapter.transform(
                steer_unit=float(ppo_action[0]),
                speed_unit=float(ppo_action[1]),
                dt=self.dt,
            )

            raw_policy_steer = float(
                stable_cmd["raw_steer_unit"]
            )
            raw_policy_speed_unit = float(
                stable_cmd["raw_speed_unit"]
            )
            requested_steer = float(
                stable_cmd["steer_cmd"]
            )
            requested_speed = float(
                stable_cmd["speed_cmd_mps"]
            )

            self._stable_prev_steer_cmd = float(
                requested_steer
            )
            self._stable_prev_speed_cmd_mps = float(
                requested_speed
            )
            ...
            # Expose policy-vs-adapter steering for diagnostics only.
            deadbanded_policy_steer = (
                0.0
                if abs(raw_policy_steer)
                < float(
                    self.stable_action_adapter
                    .steer_deadband
                )
                else float(
                    raw_policy_steer
            ...
            control_info[
                "raw_policy_speed_unit"
            ] = float(
                raw_policy_speed_unit
            )
            control_info[
                "stable_requested_steer_cmd"
            ] = float(
                requested_steer
            )
            control_info[
                "stable_requested_speed_cmd_mps"
            ] = float(
                requested_speed
            )
            control_info[
            ...
            info[
                ...
                self._episode_dynamics_dr_enabled
            )

            info[
                "vision_dr_enabled"
            ] = bool(
                self._episode_vision_dr_enabled
            )

            info[
                "sensor_dr_enabled"
            ] = bool(
                self._episode_sensor_dr_enabled
            )
            ...
            info["dr_episode_families"] = tuple(self._episode_dr_families)
            info["dr_family_budget"] = int(self.dr_family_budget)
            info["policy_speed_max_mps"] = float(self.policy_speed_max_mps)
            info["steer_deadband"] = float(self.steer_deadband)
            ...
            info["policy_speed_unit"] = float(
                raw_policy_speed_unit
            )
            info["requested_speed_cmd_mps"] = float(
                requested_speed
            )
            info["stable_filtered_steer_cmd"] = float(
                requested_steer
            )
            info["stable_filtered_speed_cmd_mps"] = float(
                requested_speed
            )
            info["teacher_steer_target_preaction"] = float(
                teacher_steer_target_unit
            )
            info["teacher_preview_error_preaction_rad"] = float(
                teacher_preview_error_rad
            )
            info["teacher_lookahead_preaction_m"] = float(
                teacher_lookahead_distance_m
            )
            ...
            try:
                _loc = self.vehicle.get_location()
                info["world_x"] = float(_loc.x)
                info["world_y"] = float(_loc.y)
                info["world_z"] = float(_loc.z)
            except Exception:
                info["world_x"] = 0.0
                info["world_y"] = 0.0
                info["world_z"] = 0.0
...
# Canonical class name used by train_ppo_rgb_v5_singlemap_stable.py
CarlaEnvironmentRGBV5SingleMapStable = CarlaEnvironmentRGBV5
