...
class RewardConfigV3:
    ...
    # SIM-TO-REAL SAFE: control loop is frozen at 50 Hz.
    # Positive progress above desired_speed * dt must NOT earn extra reward,
    # otherwise PPO is mathematically rewarded for overspeeding.
    control_dt_s: float = 0.020
    cap_positive_progress_to_target: bool = True
...
class RewardV3:
    ...
    def compute(
        ...
        progress_for_reward = float(progress_delta_m)
        if (
            self.cfg.cap_positive_progress_to_target
            and progress_for_reward > 0.0
        ):
            target_progress_cap_m = max(
                0.0,
                float(target_speed_mps) * float(self.cfg.control_dt_s),
            )
            progress_for_reward = min(
                progress_for_reward,
                target_progress_cap_m,
            )
