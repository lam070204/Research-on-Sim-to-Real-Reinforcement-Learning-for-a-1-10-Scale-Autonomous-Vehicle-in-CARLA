# -*- coding: utf-8 -*-
"""
CARLA RGB V5 - CLEAN / FLATTENED

Một environment production duy nhất cho PPO V5 train lại từ đầu.
...
class RecoveryScenarioConfigV5(object):
    ...
    def __init__(self):
        # ONE-SPAWN curriculum: spawn perturbation is permanently disabled.
...
class CarlaEnvironmentRGBV5(object):
    ...
    def __init__(
        ...
        # Recovery-state curriculum switch.
        recovery_scenarios_enabled=True,
    ):
        self.client = client
        self.world = world
        self.town = town
        ...
        if len(self.safe_spawn_indices) != 1:
            raise RuntimeError(
                "V5 ONE-SPAWN requires exactly ONE spawn, got {}.".format(
                    [i + 1 for i in self.safe_spawn_indices]
                )
            )

        # Fixed spawn forever: no shuffle, no round-robin.
    ...
    def _resolve_spawn_indices(
        ...
        if safe_spawn_numbers is None:
            # ONE-SPAWN default: first CARLA spawn only.
            return [0]
    ...
    def _next_base_spawn_transform(self):
        index = int(self.safe_spawn_indices[0])
    ...
    def _next_spawn_transform(self):
        # ONE-SPAWN guarantee: recovery never changes spawn position/heading.
    ...
    def reset(self):
        try:
            ...
            # Sample all episode-level scenario/DR before spawn.
            ...
            print(
                ...
                "scenario=NOMINAL_ONE_SPAWN | offset={:+.3f}m | heading={:+.1f}deg | "
                "recovery_selected={} min_step={} stable_need={} | "
...
# Canonical import used by trainer/evaluator.
