...
from dynamics_calibration.common.longitudinal_tune_v2 import (
    apply_longitudinal_tune_v2,
    ActionControllerV3TuneV2,
)
...
def spawn_vehicle(world, spawn_number):
    ...
    tune_info = apply_longitudinal_tune_v2(vehicle)
    world.tick()
    print(
        "    TUNE V2 | mass={:.1f} | torque={} | brake={} | "
        "autobox={} | gear_switch={:.3f}s".format(
            tune_info["mass_kg"],
            tune_info["torque_curve"],
            tune_info["max_brake_torque"],
            tune_info["use_gear_autobox"],
            tune_info["gear_switch_time_s"],
        )
    )
    world.tick()
...
def run_neutral_stop_segment(
    ...
    """
    Stop-between dành cho Tune V2 fixed manual gear.

    Giữ xe ở MANUAL NEUTRAL (gear=0) + brake=1 trong đoạn nghỉ.
    Không dùng ActionControllerV3TuneV2 ở đây, vì controller đó luôn ép gear=1.
    Giữ gear=1 + full brake lâu có thể làm drivetrain PhysX rơi vào trạng thái
    không kéo được xe ở segment kế tiếp trên custom CARLA 0.9.13-dirty.
    """
    ...
    for i in range(n_ticks):
        ...
        rows.append({
            "segment": "stop_between",
            "jetson_elapsed_s": float((i + 1) * DT),
            "time_ms": int(round(total_elapsed_s * 1000.0)),
            "speed_cmd_mps": 0.0,
            "speed_mps": float(vehicle_planar_speed_real_mps(vehicle)),
            "motor_pwm": "",
            ...
            "failsafe": 0,
            "sim_frame": int(frame),
            "requested_speed_cmd_mps": 0.0,
            "applied_speed_cmd_mps": 0.0,
            "carla_speed_raw_mps": float(
                vehicle_planar_speed_real_mps(vehicle)
                * CARLA_GEOMETRY_SCALE
            ),
            "target_speed_carla_mps": 0.0,
            "throttle": 0.0,
            "brake": 1.0,
            "carla_steer": 0.0,
            "gear": 0,
...
def main():
    ...
    print("=" * 88)
    print("GĐ4 - TEST 3 DECELERATION / COAST - SMALL MAP - TUNE V2 NEUTRAL-STOP")
    print("=" * 88)
    ...
    try:
        ...
        controller = ActionControllerV3TuneV2(vehicle)
        ...
        for initial_cmd, final_cmd in CASES:
            ...
            # 1) Nghỉ ở neutral, KHÔNG ép gear=1 + full brake.
            total_elapsed_s = run_neutral_stop_segment(
                world,
                vehicle,
                rows,
                STOP_BETWEEN_S,
            ...
            neutral_control = vehicle.get_control()
            print(
                "    STOP READY | manual={} | gear={} | speed={:.4f}".format(
                    bool(neutral_control.manual_gear_shift),
                    int(neutral_control.gear),
                    float(vehicle_planar_speed_real_mps(vehicle)),
                )
            )

            # 2) Ngay trước pre-speed mới vào fixed gear 1.
            # Cách này giống Test 2 V2 - bài đã chạy thành công.
            controller.reset()
            world.tick()

            control_after_reset = vehicle.get_control()
            print(
                "    DRIVE READY | manual={} | gear={} | speed={:.4f}".format(
                    bool(control_after_reset.manual_gear_shift),
                    int(control_after_reset.gear),
                    float(vehicle_planar_speed_real_mps(vehicle)),
                )
            )

            if (
                not bool(control_after_reset.manual_gear_shift)
                or int(control_after_reset.gear) != 1
            ):
                raise RuntimeError(
                    "Tune V2 drivetrain chưa sẵn sàng: manual={} gear={}".format(
                        bool(control_after_reset.manual_gear_shift),
                        int(control_after_reset.gear),
                    )
                )
