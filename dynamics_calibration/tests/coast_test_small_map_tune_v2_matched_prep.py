...
def spawn_vehicle(world, spawn_number):
    ...
    world.tick()
    print(
        "    TUNE V2 | mass={:.1f} | torque={} | brake={}".format(
            tune_info["mass_kg"],
...
def main():
    ...
    print("=" * 88)
    print("GĐ4 - TEST 3 DECELERATION / COAST - SMALL MAP - TUNE V2 MATCHED-PREP")
    print("=" * 88)
    ...
    try:
        ...
        for initial_cmd, final_cmd in CASES:
            ...
            # Match Test 2 V2 exactly:
            # commit fixed manual gear=1 before the 3 s stopped preparation.
            world.tick()

            control_ready = vehicle.get_control()
            print(
                "    PREP READY | manual={} | gear={} | speed={:.4f}".format(
                    bool(control_ready.manual_gear_shift),
                    int(control_ready.gear),
                    float(vehicle_planar_speed_real_mps(vehicle)),
                )
            )

            if (
                not bool(control_ready.manual_gear_shift)
                or int(control_ready.gear) != 1
            ):
                raise RuntimeError(
                    "Fixed gear chưa sẵn sàng: manual={} gear={}".format(
                        bool(control_ready.manual_gear_shift),
                        int(control_ready.gear),
                    )
                )
            ...
            pre_segment_name = "pre_{:.2f}_to_{:.2f}".format(
                initial_cmd,
                final_cmd,
            )
            pre_case_rows = [
                row for row in rows
                if row["segment"] == pre_segment_name
            ]
            if pre_case_rows:
                last_window = pre_case_rows[-25:]
                last_mean = sum(
                    float(row["speed_mps"])
                    for row in last_window
                ) / float(len(last_window))
                last_std = (
                    sum(
                        (float(row["speed_mps"]) - last_mean) ** 2
                        for row in last_window
                    ) / float(len(last_window))
                ) ** 0.5
                print(
                    "    PRE SPEED | final={:.4f} | last0.5s={:.4f} ± {:.4f}".format(
                        float(pre_case_rows[-1]["speed_mps"]),
                        float(last_mean),
                        float(last_std),
                    )
                )
