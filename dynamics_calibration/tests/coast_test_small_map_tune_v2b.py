...
def main():
    ...
    print("=" * 88)
    print("GĐ4 - TEST 3 DECELERATION / COAST - SMALL MAP - TUNE V2B AUTOBOX ZERO-SHIFT")
    print("=" * 88)
    ...
    try:
        ...
        for initial_cmd, final_cmd in CASES:
            ...
            if pre_case_rows:
                ...
                m = sum(float(r["speed_mps"]) for r in last_window) / float(len(last_window))
                s = (
                    sum((float(r["speed_mps"]) - m) ** 2 for r in last_window)
                    / float(len(last_window))
                ) ** 0.5
                last = pre_case_rows[-1]
                print(
                    "    PRE SPEED | final={:.4f} | last0.5s={:.4f} ± {:.4f} | "
                    "gear={} | manual={}".format(
                        float(last["speed_mps"]),
                        float(m),
                        float(s),
                        int(last["gear"]),
                        bool(int(last["manual_gear_shift"])),
                    )
                )
