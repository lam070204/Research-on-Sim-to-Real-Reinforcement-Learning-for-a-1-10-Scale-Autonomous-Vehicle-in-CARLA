...
def spawn_vehicle(world, spawn_number):
    ...
    world.tick()
    print(
        "    TUNE V2 PASSIVE-GEAR | mass={:.1f} | torque={} | brake={} | "
...
def main():
    ...
    print("=" * 88)
    print("GĐ4 - TEST 3 DECELERATION / COAST - SMALL MAP - TUNE V2 PASSIVE-GEAR")
    print("=" * 88)
    ...
    try:
        ...
        for initial_cmd, final_cmd in CASES:
            ...
            if pre_case_rows:
                ...
                last_row = pre_case_rows[-1]
                print(
                    "    PRE SPEED | final={:.4f} | last0.5s={:.4f} ± {:.4f} "
                    "| gear={} | manual={}".format(
                        float(last_row["speed_mps"]),
                        float(last_mean),
                        float(last_std),
                        int(last_row["gear"]),
                        bool(int(last_row["manual_gear_shift"])),
                    )
                )
