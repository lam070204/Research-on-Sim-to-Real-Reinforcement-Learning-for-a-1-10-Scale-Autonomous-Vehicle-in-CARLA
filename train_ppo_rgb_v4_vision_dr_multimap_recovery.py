# V4_VISION_DR_MULTIMAP_DELAYED_RECOVERY
...
- Recovery training is an ablation variant: clean spawn plus a short balanced
  steering disturbance in selected episodes, only after a stable-drive gate.
- Keep a different --model-name from the nominal V4 policy.
...
from simulation.carla_environment_rgb_v4_vision_dr import CarlaEnvironmentRGB
...
DEFAULT_MODEL_NAME = "automav3_rgb_v4_vision_dr"
...
def parse_args():
    ...
    # Delayed recovery-state training (same idea as the guarded V5 curriculum).
    parser.add_argument("--recovery-enabled", type=boolean_string, default=True)
    parser.add_argument("--recovery-episode-prob", type=float, default=0.30)
    parser.add_argument("--recovery-control-hz", type=float, default=50.0)
    parser.add_argument("--recovery-min-episode-step", type=int, default=700)
    parser.add_argument("--recovery-stable-ticks", type=int, default=100)
    parser.add_argument("--recovery-stable-lateral-m", type=float, default=0.040)
    parser.add_argument("--recovery-stable-heading-deg", type=float, default=5.0)
    parser.add_argument("--recovery-stable-speed-mps", type=float, default=0.40)
    parser.add_argument("--recovery-duration-min-s", type=float, default=0.10)
    parser.add_argument("--recovery-duration-max-s", type=float, default=0.16)
    parser.add_argument("--recovery-steer-min", type=float, default=0.10)
    parser.add_argument("--recovery-steer-max", type=float, default=0.18)
    parser.add_argument("--recovery-success-hold-ticks", type=int, default=8)
    parser.add_argument("--recovery-max-events", type=int, default=1)
...
def validate_args(args):
    ...
    if not (0.0 <= float(args.recovery_episode_prob) <= 1.0):
        raise ValueError("--recovery-episode-prob must be in [0,1].")
    if float(args.recovery_control_hz) <= 0.0:
        raise ValueError("--recovery-control-hz must be > 0.")
    if int(args.recovery_min_episode_step) < 0:
        raise ValueError("--recovery-min-episode-step must be >= 0.")
    if int(args.recovery_stable_ticks) <= 0:
        raise ValueError("--recovery-stable-ticks must be > 0.")
    if float(args.recovery_stable_lateral_m) < 0.0:
        raise ValueError("--recovery-stable-lateral-m must be >= 0.")
    if float(args.recovery_stable_heading_deg) < 0.0:
        raise ValueError("--recovery-stable-heading-deg must be >= 0.")
    if float(args.recovery_stable_speed_mps) < 0.0:
        raise ValueError("--recovery-stable-speed-mps must be >= 0.")
    if not (0.0 < float(args.recovery_duration_min_s)
            <= float(args.recovery_duration_max_s)):
        raise ValueError("Recovery duration range is invalid.")
    if not (0.0 <= float(args.recovery_steer_min)
            <= float(args.recovery_steer_max) <= 1.0):
        raise ValueError("Recovery steer range must satisfy 0 <= min <= max <= 1.")
    if int(args.recovery_success_hold_ticks) <= 0:
        raise ValueError("--recovery-success-hold-ticks must be > 0.")
    if int(args.recovery_max_events) <= 0:
        raise ValueError("--recovery-max-events must be > 0.")
...
class EpisodeStats:
    ...
    def add(self, action, reward, info):
        ...
        self.recovery_selected = bool(
            self.recovery_selected or recovery.get("episode_selected", False)
        )
        self.recovery_trigger_count += int(
            bool(recovery.get("trigger_event", False))
        )
        self.recovery_active_ticks += int(
            bool(recovery.get("disturbance_active", False))
        )
        self.recovery_success_count += int(
            bool(recovery.get("success_event", False))
        )
    ...
    def summary(self):
        return {
            ...
            "recovery_selected": bool(self.recovery_selected),
            "recovery_trigger_count": int(self.recovery_trigger_count),
...
def print_episode(
    ...
    print(
        ...
        "  recovery_selected={} | inject={} | active_ticks={} | success={}\n"
        "  action_std={:.4f}".format(
            ...
            summary.get("recovery_selected", False),
            summary.get("recovery_trigger_count", 0),
            summary.get("recovery_active_ticks", 0),
            summary.get("recovery_success_count", 0),
            action_std,
        )
    )
...
def write_episode(writer, episode, global_step, summary, desired_speed_mps):
    ...
    writer.add_scalar(
        "recovery/episode_selected",
        float(summary.get("recovery_selected", False)),
        global_step,
    )
    writer.add_scalar(
        "recovery/trigger_count",
        float(summary.get("recovery_trigger_count", 0)),
        global_step,
    )
    writer.add_scalar(
        "recovery/active_ticks",
        float(summary.get("recovery_active_ticks", 0)),
        global_step,
    )
    writer.add_scalar(
        "recovery/success_count",
        float(summary.get("recovery_success_count", 0)),
        global_step,
    )
...
class DelayedRecoveryTrainerV4(object):
    """Inject short exogenous steering noise only after stable nominal driving."""

    def __init__(self, args):
        self.args = args
        self.enabled = bool(args.recovery_enabled)
        self.reset_episode(0)

    def reset_episode(self, episode_index):
        self.episode_index = int(episode_index)
        # Per-episode RNG makes selection reproducible after checkpoint resume.
        self.rng = random.Random(
            int(self.args.seed) + 7919 * int(self.episode_index)
        )
        self.selected = bool(
            self.enabled
            and self.rng.random() < float(self.args.recovery_episode_prob)
        )
        self.episode_step = 0
        self.stable_ticks = 0
        self.remaining_disturbance_ticks = 0
        ...
        print(
            "RECOVERY EPISODE | episode={} | selected={} | probability={:.2f}".format(
                self.episode_index,
                self.selected,
                float(self.args.recovery_episode_prob),
            )
        )

    def before_env_step(self, policy_action):
        self.episode_step += 1
        action = np.asarray(policy_action, dtype=np.float32).reshape(-1).copy()
        if action.size != 2:
            raise ValueError(
                "V4 recovery expects action [steer, speed], received {}".format(
                    tuple(action.shape)
                )
            )

        active = self.remaining_disturbance_ticks > 0
        applied_disturbance = self.disturbance_steer if active else 0.0
        if active:
            action[0] = np.clip(
                float(action[0]) + float(applied_disturbance),
                -1.0,
                1.0,
            )
            self.remaining_disturbance_ticks -= 1

        self.ended_this_step = bool(
            active and self.remaining_disturbance_ticks == 0
        )
        return action, bool(active), float(applied_disturbance)

    def _stable_now(self, info):
        imu = info.get("imu", {}) or {}
        return bool(
            abs(float(info.get("lateral_error_m", float("inf"))))
            <= float(self.args.recovery_stable_lateral_m)
            and abs(float(info.get("heading_error_rad", float("inf"))))
            <= math.radians(float(self.args.recovery_stable_heading_deg))
            and float(imu.get("speed_mps", 0.0))
            >= float(self.args.recovery_stable_speed_mps)
        )

    def after_env_step(self, info, active, applied_disturbance):
        stable_now = self._stable_now(info)
        triggered = False
        success = False
        ...
        if self.recovery_pending:
            self.success_hold = self.success_hold + 1 if stable_now else 0
            if self.success_hold >= int(self.args.recovery_success_hold_ticks):
                self.recovery_pending = False
                self.success_hold = 0
                success = True
                print(
                    "RECOVERY SUCCESS | episode={} | step={} | event={}".format(
                        self.episode_index, self.episode_step, self.event_count
                    )
                )
        ...
        can_trigger = bool(
            ...
            and self.remaining_disturbance_ticks == 0
            and self.event_count < int(self.args.recovery_max_events)
            and self.episode_step >= int(self.args.recovery_min_episode_step)
        )

        if can_trigger:
            self.stable_ticks = self.stable_ticks + 1 if stable_now else 0
            if self.stable_ticks >= int(self.args.recovery_stable_ticks):
                duration_s = self.rng.uniform(
                    float(self.args.recovery_duration_min_s),
                    float(self.args.recovery_duration_max_s),
                )
                duration_ticks = max(
                    1,
                    int(round(duration_s * float(self.args.recovery_control_hz))),
                )
                magnitude = self.rng.uniform(
                    float(self.args.recovery_steer_min),
                    float(self.args.recovery_steer_max),
                )
                # Deterministic alternating sign keeps left/right disturbances balanced.
                sign = -1.0 if (self.episode_index + self.event_count) % 2 else 1.0
                self.disturbance_steer = float(sign * magnitude)
                self.remaining_disturbance_ticks = int(duration_ticks)
                self.event_count += 1
                self.stable_ticks = 0
                triggered = True
                print(
                    "RECOVERY INJECT | episode={} | trigger_step={} | "
                    "duration={} ticks | steer={:+.3f} | event={}/{}".format(
                        self.episode_index,
                        self.episode_step,
                        duration_ticks,
                        self.disturbance_steer,
                        self.event_count,
                        int(self.args.recovery_max_events),
                    )
                )
        elif not self.recovery_pending and not active:
            self.stable_ticks = 0

        return {
            "enabled": bool(self.enabled),
            "episode_selected": bool(self.selected),
            "episode_step": int(self.episode_step),
            "disturbance_active": bool(active),
            "disturbance_steer": float(applied_disturbance),
            "trigger_event": bool(triggered),
            "success_event": bool(success),
            "event_count": int(self.event_count),
            "recovery_pending": bool(self.recovery_pending),
            "stable_gate_ticks": int(self.stable_ticks),
        }
...
def train(args, env, agent, writer):
    global_step = 0
    episode = 0
    recovery_controller = DelayedRecoveryTrainerV4(args)
    ...
    recovery_controller.reset_episode(episode + 1)
    stats = EpisodeStats()

    print(
        "\nTRAIN START | model={} | step={}/{} | session_end={} | rollout={} | "
        "desired_speed={:.3f} | curriculum={} | recovery={} | recovery_prob={:.2f}".format(
            args.model_name,
            global_step,
            ...
            desired_speed,
            args.curriculum,
            args.recovery_enabled,
            args.recovery_episode_prob,
        )
    )
    ...
    while global_step < session_end_step:
        ...
        applied_action, disturbance_active, disturbance_steer = (
            recovery_controller.before_env_step(action)
        )
        ...
        info["recovery"] = recovery_controller.after_env_step(
            info=info,
            active=disturbance_active,
            applied_disturbance=disturbance_steer,
        )
...
def runner():
    ...
    try:
        ...
        env = CarlaEnvironmentRGB(
            ...
            # GĐ6_TRAIN_DOMAIN_RANDOMIZATION_INTEGRATED
