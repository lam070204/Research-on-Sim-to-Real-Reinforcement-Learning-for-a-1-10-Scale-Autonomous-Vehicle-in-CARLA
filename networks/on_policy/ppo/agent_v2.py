...
class PPOAgent(object):
    ...
    def learn(self):
        ...
        print("\n===== PPO DEBUG BEFORE LEARN =====")
        print("old_states shape:", old_states.shape)
        print("old_states finite:", torch.isfinite(old_states).all().item())
        print("old_states NaN:", torch.isnan(old_states).sum().item())
        print("old_states Inf:", torch.isinf(old_states).sum().item())

        print("old_actions finite:", torch.isfinite(old_actions).all().item())
        print("old_logprobs finite:", torch.isfinite(old_logprobs).all().item())

        actor_params_finite = all(
            torch.isfinite(p).all().item()
            for p in self.policy.actor.parameters()
        )
        print("policy actor params finite:", actor_params_finite)

        with torch.no_grad():
            test_mean = self.policy.actor(old_states)

        print("actor mean finite:", torch.isfinite(test_mean).all().item())
        print("actor mean NaN:", torch.isnan(test_mean).sum().item())
        print("==================================\n")
