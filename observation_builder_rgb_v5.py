...
from encoder_runtime_rgb_v5 import EncodeRGBV5, LATENT_DIM_V5


OBSERVATION_DIM_V5 = 100
PROPRIO_DIM_V3 = 5
...
class ObservationBuilderRGBV5:
    def __init__(self, encoder=None):
        self.encoder = encoder if encoder is not None else EncodeRGBV5()

        if self.encoder.latent_dim != LATENT_DIM_V5:
            raise ValueError(
    ...
    def build(
        ...
        if tuple(latent.shape) != (LATENT_DIM_V5,):
            raise RuntimeError(
        ...
        if tuple(observation.shape) != (OBSERVATION_DIM_V5,):
            raise RuntimeError(
    ...
    def unpack_proprio(observation):
        ...
        if tuple(obs.shape) != (OBSERVATION_DIM_V5,):
            raise ValueError(
