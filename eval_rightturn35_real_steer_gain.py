...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "EVAL ONLY: multiply PPO steering by this gain before env.step(). "
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "EVAL ONLY: clip gained steering to [-cap,+cap] before env.step()."
        ),
    )
    parser.add_argument(
        "--dynamics-dr",
...
def validate_args(args):
    ...
    if not (0.0 < float(args.eval_steer_gain) <= 5.0):
        raise ValueError("--eval-steer-gain must be in (0,5].")

    if not (0.0 < float(args.eval_steer_cap) <= 1.0):
        raise ValueError("--eval-steer-cap must be in (0,1].")
...
def evaluate(args, env, agent, writer):
    ...
    print(
        "EVAL REAL-STEER EMULATION | gain={:.3f} | cap={:.3f} | speed_cap={:.3f}".format(
            float(args.eval_steer_gain),
            float(args.eval_steer_cap),
            float(args.policy_speed_cap),
        )
    )

    while timestep < args.test_timesteps:
        raw_action = agent.get_action(
            observation,
            train=False,
        )

        # EVAL ONLY: emulate Jetson real steering:
        # raw PPO steer -> * gain -> clip by eval steer cap -> env.step().
        action = np.asarray(raw_action, dtype=np.float32).copy()
        raw_steer = float(action[0])
        gained_steer = raw_steer * float(args.eval_steer_gain)
        applied_steer = float(
            np.clip(
                gained_steer,
                -float(args.eval_steer_cap),
        ...
        if (
            int(args.print_every_steps) > 0
            and (timestep + 1) % int(args.print_every_steps) == 0
        ):
            print(
                "[EVAL-REAL-STEER] step={:05d} | "
                "PPO_RAW={:+.4f} | GAIN={:.2f} -> {:+.4f} | "
                "APPLIED={:+.4f} | PPO_SPEED={:.4f}".format(
                    timestep + 1,
                    raw_steer,
                    float(args.eval_steer_gain),
                    gained_steer,
