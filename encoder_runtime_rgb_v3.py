...
class EncodeRGBV3:
    def __init__(self, latent_dim=LATENT_DIM_V3, device=None):
        self.latent_dim = int(latent_dim)

        if self.latent_dim != LATENT_DIM_V3:
            raise ValueError(
                "V3 latent_dim phải là {}, nhận được {}.".format(
                    LATENT_DIM_V3,
                    self.latent_dim,
                )
            )
        ...
        if not os.path.isfile(ENCODER_V3_PATH):
            raise FileNotFoundError(
                "Không tìm thấy encoder V3: {}".format(
                    ENCODER_V3_PATH
                )
            )
        ...
        print(
            "RGB V3 encoder loaded | latent={} | device={} | model={}".format(
                self.latent_dim,
                self.device,
                ENCODER_V3_PATH,
            )
    ...
    def _image_to_tensor(self, image_rgb):
        ...
        if image_np.ndim != 3:
            raise ValueError(
                "RGB V3 phải có HxWxC, nhận được shape {}.".format(
                    image_np.shape
                )
            )
        ...
        if tuple(image_np.shape) != expected_shape:
            raise ValueError(
                "RGB V3 phải có shape {}, nhận được {}.".format(
                    expected_shape,
