...
from simulation.carla_environment_rgb_v5_simple import (
    CarlaEnvironmentRGBV5Simple as CarlaEnvironmentRGBV5MultiMap,
...
STAGE_NAMES = {
    ...
    1: "UNUSED_SIMPLE_1",
    2: "UNUSED_SIMPLE_2",
    3: "MULTIMAP_SIM2REAL_SIMPLE",
}

MODEL_NAME = "automav5_rgb_simple"
TRAINER_VERSION = "TRAINER_PPO_V5_SIMPLE_FAST_1_ORIGINAL_REWARD_MIXED_DR"
...
STATE_NAME = "training_state_v5_simple.json"
CONTROL_HZ = 50.0
...
def parse_args():
    ...
    p.add_argument("--dr-episode-prob", type=float, default=0.75,
                   help="Probability that an episode uses FULL dynamics+vision+sensor+weather DR. Remaining episodes are exact nominal.")
    ...
    p.add_argument("--canary-every-steps", type=int, default=100000)
    p.add_argument("--canary-max-steps", type=int, default=600)
    ...
    p.add_argument("--initial-map-validation", type=boolean_string, default=False)
    p.add_argument("--map-validation-every-steps", type=int, default=300000)
    p.add_argument("--mastery-max-steps", type=int, default=750)
    ...
    p.add_argument("--robust-validation-every-steps", type=int, default=300000)
    p.add_argument("--robust-max-steps", type=int, default=600)
    p.add_argument("--robust-spawns", type=parse_spawn_numbers,
                   default=parse_spawn_numbers("1,3"))
    p.add_argument("--robust-repeats", type=int, default=1)
    ...
    p.add_argument("--dr-min-full-ratio", type=float, default=0.60)
    ...
    p.add_argument("--failure-report-every-steps", type=int, default=25000)
    p.add_argument("--failure-persistent-age-steps", type=int, default=100000)
...
def validate_args(a):
    ...
    if not (0.10 <= float(a.desired_speed) <= 1.0):
        raise ValueError("--desired-speed must be in [0.10, 1.0] real-equivalent m/s")
    if not (0.0 <= float(a.dr_episode_prob) <= 1.0):
        raise ValueError("--dr-episode-prob must be in [0,1]")
    ...
    for name in (
        ...
        "scheduler_cycle_slots", "failure_report_every_steps",
    ):
...
def stage_config(stage, args, repair_mode=False, recovery_level=1):
    # SIMPLE FAST: no curriculum stages. Every training step is multimap and
    # all configured DR families are available from step 0. The environment
    # mixes FULL_DR and exact NOMINAL episodes using --dr-episode-prob.
    return dict(
        name=STAGE_NAMES[3],
        multimap=True,
        recovery=False,
...
def assigned_maps_for_block(stage, block_index, worker_count, weights, args):
    """Coverage-first 50k scheduler with explicit green-map priority.

    For two workers, every 7 blocks (350k transitions) covers all 11 maps,
    while xanhphai and mapxanhdam each receive an extra slot. The final spare
    slot goes to the currently hardest non-green/non-stress map by adaptive
    weight. This removes random starvation while still repairing hard maps.
    """
    block_index = int(block_index)
    worker_count = max(1, int(worker_count))

    green_r = ANCHOR_MAPS[0]
    green_l = ANCHOR_MAPS[1]
    dancu_r = AUX_OVAL_MAPS[0]
    dancu_l = AUX_OVAL_MAPS[1]
    lonxon = AUX_OVAL_MAPS[2]
    lonxon1 = AUX_OVAL_MAPS[3]
    rung_r = AUX_OVAL_MAPS[4]
    rung_l = AUX_OVAL_MAPS[5]
    mapto_l = AUX_OVAL_MAPS[6]
    mapto_r = AUX_OVAL_MAPS[7]

    hard_candidates = [
        m for m in ALL_MAPS
        if m not in (green_r, green_l, LEFT_ONLY_STRESS_MAP)
    ]
    hardest = max(hard_candidates, key=lambda m: float(weights.get(m, 0.0)))
    ...
    pairs = [
        ...
        (rung_r, rung_l),
        (mapto_l, mapto_r),
        (LEFT_ONLY_STRESS_MAP, green_r),
        (green_l, hardest),
    ]
    pair = pairs[block_index % len(pairs)]

    if worker_count == 1:
        # Alternate the two slots across consecutive cycles when only one CARLA
        # server is used; two-worker mode is the intended fast configuration.
        cycle = block_index // len(pairs)
        return [pair[cycle % 2]]

    result = list(pair[:worker_count])
    while len(result) < worker_count:
        result.append(ALL_MAPS[(block_index + len(result)) % len(ALL_MAPS)])
    return result
...
class RecoveryAudit(object):
    ...
    def gate(self, args, level_gate=False):
        if float(getattr(args, "recovery_episode_prob", 0.0)) <= 0.0:
            return True, [], {
                "disabled": True,
                "injected_pos": int(self.injected_pos),
                "injected_neg": int(self.injected_neg),
...
class DRExposureTracker(object):
    ...
    def add_rollout(self, map_path, samples, dynamics, vision, sensor, dr_step_counts=None):
        samples = int(samples)
        self.total_steps += samples
        self.map_steps[map_path] = int(self.map_steps.get(map_path, 0)) + samples

        counts = dr_step_counts if isinstance(dr_step_counts, dict) else None
        if counts is None:
            dyn_steps = samples if dynamics else 0
            vis_steps = samples if vision else 0
            sen_steps = samples if sensor else 0
            full_steps = samples if (dynamics and vision and sensor) else 0
        else:
            dyn_steps = int(counts.get("dynamics", 0))
            vis_steps = int(counts.get("vision", 0))
            sen_steps = int(counts.get("sensor", 0))
            full_steps = int(counts.get("full", 0))

        self.dynamics_steps += dyn_steps
        self.vision_steps += vis_steps
        self.sensor_steps += sen_steps
        self.full_steps += full_steps
        self.map_full_dr_steps[map_path] = int(self.map_full_dr_steps.get(map_path, 0)) + full_steps
...
class FailureClusterTracker(object):
    ...
    def _key(self, event):
        # IMPORTANT: cluster by MAP + PHYSICAL LOCATION, not by spawn.
        # A true dead spot reached from different spawns must collapse into one cluster.
        gx = int(round(float(event["world_x"]) / self.grid_size))
        gy = int(round(float(event["world_y"]) / self.grid_size))
        return "{}|{}|{}".format(event["map_path"], gx, gy)
    ...
    def add(self, event):
        ...
        rec = self.cells.setdefault(key, {
            ...
            "sum_heading_deg": 0.0,
            "sum_ppo_steer": 0.0,
            "sum_actual_steer": 0.0,
            "sum_physical_steer": 0.0,
            "sum_speed": 0.0,
            "sum_speed_cmd": 0.0,
            "sum_camera_lag": 0.0,
            "camera_lag_count": 0,
        ...
        rec["sum_lat"] += abs(float(event.get("lateral_error_m", 0.0)))
        rec["sum_heading_deg"] += abs(float(event.get("heading_error_deg", 0.0)))
        rec["sum_ppo_steer"] += float(event.get("ppo_steer", 0.0))
        rec["sum_actual_steer"] += float(event.get("actual_steer", 0.0))
        rec["sum_physical_steer"] += float(event.get("physical_steer", 0.0))
        rec["sum_speed"] += float(event.get("speed_mps", 0.0))
        rec["sum_speed_cmd"] += float(event.get("actual_speed_cmd_mps", 0.0))
        lag = event.get("camera_frame_lag")
        if lag is not None:
            rec["sum_camera_lag"] += float(lag)
            rec["camera_lag_count"] += 1
        if bool(event.get("dr_active", False)):
            rec["dr_failures"] += 1
        else:
            rec["nominal_failures"] += 1
        spawn = str(int(event.get("spawn_number", 0)))
        rec["spawn_counts"][spawn] = int(rec["spawn_counts"].get(spawn, 0)) + 1
        step = int(event.get("train_step", 0))
    ...
    def top_clusters(self, limit=10, persistent_age_steps=100000, alert_count=5):
        ...
        for rec in items[:int(limit)]:
            ...
            item["mean_x"] = float(rec.get("sum_x", 0.0)) / n
            item["mean_y"] = float(rec.get("sum_y", 0.0)) / n
            item["mean_progress_m"] = float(rec.get("sum_progress", 0.0)) / n
            item["mean_lat_m"] = float(rec.get("sum_lat", 0.0)) / n
            item["mean_heading_deg"] = float(rec.get("sum_heading_deg", 0.0)) / n
            item["mean_ppo_steer"] = float(rec.get("sum_ppo_steer", 0.0)) / n
            item["mean_actual_steer"] = float(rec.get("sum_actual_steer", 0.0)) / n
            item["mean_physical_steer"] = float(rec.get("sum_physical_steer", 0.0)) / n
            item["mean_speed_mps"] = float(rec.get("sum_speed", 0.0)) / n
            item["mean_speed_cmd_mps"] = float(rec.get("sum_speed_cmd", 0.0)) / n
            lc = max(int(rec.get("camera_lag_count", 0)), 1)
            item["mean_camera_lag"] = float(rec.get("sum_camera_lag", 0.0)) / lc
            first = rec.get("first_step")
            last = rec.get("last_step")
            age = int(last - first) if first is not None and last is not None else 0
            item["age_steps"] = age
            item["status"] = (
                "PERSISTENT" if n >= int(alert_count) and age >= int(persistent_age_steps)
                else ("REPEATED" if n >= int(alert_count) else "NEW")
            )
            out.append(item)
        return out
...
class WorkerRuntime(object):
    ...
    def ensure_train_env(self, command):
        signature = (
            ...
            round(float(command.get("dr_episode_prob", self.cfg.get("dr_episode_prob", 0.75))), 4),
        ...
        self.env = CarlaEnvironmentRGBV5MultiMap(
            ...
            dr_episode_probability=float(command.get("dr_episode_prob", self.cfg.get("dr_episode_prob", 0.75))),
    ...
    def collect(self, command):
        ...
        dr_step_counts = {"dynamics": 0, "vision": 0, "sensor": 0, "full": 0, "nominal": 0}
        ...
        for local_i in range(requested_steps):
            ...
            dyn_eff = bool(info.get("dynamics_dr_enabled", False))
            vis_eff = bool(info.get("vision_dr_enabled", False))
            sen_eff = bool(info.get("sensor_dr_enabled", False))
            dr_step_counts["dynamics"] += int(dyn_eff)
            dr_step_counts["vision"] += int(vis_eff)
            dr_step_counts["sensor"] += int(sen_eff)
            dr_step_counts["full"] += int(dyn_eff and vis_eff and sen_eff)
            dr_step_counts["nominal"] += int(not (dyn_eff or vis_eff or sen_eff))
            ...
            if done:
                ...
                if reason in ("offroad", "collision", "stuck", "overspeed"):
                    ...
                    failure_events.append({
                        ...
                        "train_step": int(command.get("global_step_start", 0)) + int(local_i),
                        "ppo_steer": float(info.get("ppo_action", (action[0], action[1]))[0]),
                        "ppo_speed_cmd_mps": float(info.get("ppo_action", (action[0], action[1]))[1]),
                        "actual_steer": float(info.get("control", {}).get("actual_steer_cmd", action[0])),
                        "actual_speed_cmd_mps": float(info.get("control", {}).get("actual_speed_cmd_mps", action[1])),
                        "physical_steer": float(info.get("control", {}).get("dr_physical_steer_cmd", info.get("control", {}).get("actual_steer_cmd", action[0]))),
                        "speed_mps": float(info.get("speed_mps", 0.0)),
                        "dr_active": bool(info.get("dr_episode_active", False)),
                        "steer_gain_positive": float(info.get("domain_randomization", {}).get("steer_gain_positive", 0.0)),
                        "steer_gain_negative": float(info.get("domain_randomization", {}).get("steer_gain_negative", 0.0)),
                        "extra_command_delay_ticks": int(info.get("domain_randomization", {}).get("extra_command_delay_ticks", 0)),
                        "cloudiness": float(info.get("vision_sensor_dr", {}).get("cloudiness", 0.0)),
                        "sun_altitude_angle": float(info.get("vision_sensor_dr", {}).get("sun_altitude_angle", 0.0)),
                    })
                    if bool(self.cfg.get("latent_audit", True)):
        ...
        return {
            ...
            "dr_step_counts": dict(dr_step_counts),
        }
    ...
    def evaluate_policy(self, command):
        ...
        env = CarlaEnvironmentRGBV5MultiMap(
            ...
            safe_spawn_numbers=list(command["safe_spawns"]),
            desired_speed_mps=float(command.get("desired_speed", self.cfg.get("desired_speed", 0.50))),
            ...
            dr_episode_probability=(
                1.0 if bool(command.get("dynamics_dr", False) or command.get("vision_dr", False) or command.get("sensor_dr", False))
                else 0.0
            ),
...
def worker_configs(args):
    ...
    for worker_id in range(int(args.workers)):
        ...
        result.append({
            ...
            "dr_episode_prob": float(args.dr_episode_prob),
            "desired_speed": float(args.desired_speed),
...
def print_recovery_audit(audit):
    print("  RECOVERY INJECTION | DISABLED (recovery state remains diagnostic only)")
...
def main():
    ...
    if int(stage) != 3:
        print("SIMPLE STAGE NORMALIZE | {} -> MULTIMAP_SIM2REAL_SIMPLE".format(stage))
        stage = 3
        stage_start_step = int(global_step)
        consecutive_anchor_passes = 0
        repair_until_step = 0

    # SIMPLE trainer is Stage3-style from the first transition.
    if bool(args.force_stage3_now):
        print("SIMPLE FAST | --force-stage3-now ignored: already Stage3-style from step 0")
    ...
    try:
        ...
        print("=" * 118)
        print("PPO V5 SIMPLE FAST TRAINER | FRESH / GLOBAL ADAPTIVE MULTIMAP")
        print("=" * 118)
        ...
        print("scheduler         : COVERAGE-FIRST 50K BLOCKS + GREEN PRIORITY + HARD-MAP REPAIR SLOT")
        ...
        print("reward            : speed_factor x center_factor x heading_factor (original-repo style)")
        print("DR mix            : {:.0f}% FULL DR / {:.0f}% exact NOMINAL from step 0".format(
            100.0 * args.dr_episode_prob,
            100.0 * (1.0 - args.dr_episode_prob),
        ))
        print("recovery inject   : OFF")
        print("=" * 118)
        ...
        next_failure_report_step = ((global_step // int(args.failure_report_every_steps)) + 1) * int(args.failure_report_every_steps)
        ...
        # "switch every block" strict instead of allowing a validation/failure
        ...
        while global_step < target_step:
            ...
            if scheduler_key != scheduler_cache_key:
                ...
                if int(stage) >= 2:
                    ...
                    print("  HARD-SLOT WEIGHTS | " + " | ".join(
                        "{}={:.1f}%".format(map_short(m), 100.0 * scheduler_block_weights[m])
                        for m in ordered
                    ))
                    print("  RULE        | coverage first; green maps get bonus slots; adaptive weight only chooses repair slot")
            ...
            for msg in messages:
                exposure.add_rollout(
                    ...
                    dr_step_counts=msg.get("dr_step_counts")
                )
            ...
            if global_step >= next_failure_report_step:
                clusters_now = failure_clusters.top_clusters(
                    5, args.failure_persistent_age_steps, args.failure_cluster_alert_count
                )
                print("  DEAD-SPOT REPORT | step={} | clusters={}".format(global_step, len(failure_clusters.cells)))
                if not clusters_now:
                    print("    none")
                for rec in clusters_now:
                    print("    {:14s} {:10s} count={} xy=({:.2f},{:.2f}) spawns={} DR/NOM={}/{} age={}k | steer={:+.3f} phys={:+.3f} speed_cmd={:.3f} v={:.3f} lat={:.3f} head={:.1f} lag={:.2f}".format(
                        map_short(rec["map_path"]), rec["status"], rec["count"],
                        rec["mean_x"], rec["mean_y"], rec.get("spawn_counts", {}),
                        rec.get("dr_failures", 0), rec.get("nominal_failures", 0),
                        int(rec.get("age_steps", 0) / 1000), rec.get("mean_actual_steer", 0.0),
                        rec.get("mean_physical_steer", 0.0), rec.get("mean_speed_cmd_mps", 0.0),
                        rec.get("mean_speed_mps", 0.0), rec.get("mean_lat_m", 0.0),
                        rec.get("mean_heading_deg", 0.0), rec.get("mean_camera_lag", 0.0)
                    ))
                while next_failure_report_step <= global_step:
                    next_failure_report_step += int(args.failure_report_every_steps)
            ...
            if anchor_pass:
                ...
                if recovery_pass and float(args.recovery_episode_prob) > 0.0:
                ...
                current_checkpoint = str(rejected)
                print("ANCHOR CANDIDATE REJECTED | {}".format(rejected))
                print("SIMPLE FAST | NO AUTO-ROLLBACK | current policy continues learning so persistent dead spots can actually be repaired")
            ...
            top_clusters = failure_clusters.top_clusters(5, args.failure_persistent_age_steps, args.failure_cluster_alert_count)
            if top_clusters:
                print("  FAILURE CLUSTERS")
                for rec in top_clusters:
                    print("    {:14s} {:10s} count={} mean=({:.2f},{:.2f}) prog={:.2f}m | spawns={} | DR/NOM={}/{} | steer ppo/actual/phys={:+.3f}/{:+.3f}/{:+.3f} | speed cmd/v={:.3f}/{:.3f} | lat={:.3f} head={:.1f}deg lag={:.2f} | age={}k".format(
                        map_short(rec["map_path"]), rec["status"], rec["count"],
                        rec["mean_x"], rec["mean_y"], rec["mean_progress_m"],
                        rec.get("spawn_counts", {}), rec.get("dr_failures", 0), rec.get("nominal_failures", 0),
                        rec.get("mean_ppo_steer", 0.0), rec.get("mean_actual_steer", 0.0), rec.get("mean_physical_steer", 0.0),
                        rec.get("mean_speed_cmd_mps", 0.0), rec.get("mean_speed_mps", 0.0),
                        rec.get("mean_lat_m", 0.0), rec.get("mean_heading_deg", 0.0), rec.get("mean_camera_lag", 0.0),
                        int(rec.get("age_steps", 0) / 1000)
                    ))
        ...
        if not args.smoke_test:
            ...
            print("  RECOVERY         : DISABLED (diagnostic state only)")
